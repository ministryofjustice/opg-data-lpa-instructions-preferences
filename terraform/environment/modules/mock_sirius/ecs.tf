...
data "aws_ecr_repository" "mock_sirius_web" {
  provider = aws.management
  name     = "integrations/lpa-iap-mock-sirius-web"
}
...
resource "aws_ecs_task_definition" "lpa_iap_mock_sirius" {
  ...
  cpu                      = 512
  memory                   = 1024
  container_definitions    = "[${local.lpa_iap_mock_sirius_web}, ${local.lpa_iap_mock_sirius}]"
...
locals {
  ...
  lpa_iap_mock_sirius_web = jsonencode({
    cpu       = 1,
    essential = true,
    image     = "${data.aws_ecr_repository.mock_sirius_web.repository_url}:${var.image_tag}",
    name      = "mock-sirius-web",
    logConfiguration = {
      logDriver = "awslogs",
      options = {
        awslogs-group         = aws_cloudwatch_log_group.lpa_iap.name,
        awslogs-region        = "eu-west-1",
        awslogs-stream-prefix = "lpa-iap-mock-sirius-web-${var.environment}"
      }
    },
  })
}
