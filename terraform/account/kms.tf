...
resource "aws_kms_alias" "s3" {
  name          = "alias/lpa-iap-s3-${terraform.workspace}"
...
resource "aws_kms_key" "cloudwatch_logs" {
  description             = "Instructions and Preferences cloudwatch logs for ${local.environment}"
  deletion_window_in_days = 10
  enable_key_rotation     = true
...
resource "aws_kms_alias" "cloudwatch_logs_alias" {
  name          = "alias/lpa-iap-logs-${local.environment}"
