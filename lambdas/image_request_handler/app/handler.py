...
class ImageRequestHandler:
    ...
    def process_request(self) -> dict:
        ...
        try:
            ...
            logger.info(message['status'], extra=get_event_details_for_logs(self.event, status=status_code))
    ...
    def check_image_statuses(self, images_to_check) -> dict:
        ...
        for index in range(self.continuation_sheet_instructions_count):
            ...
            try:
                logger.debug(f'Checking continuation instruction image status for: {continuation_instruction_image}')
                image_statuses[continuation_instruction_image] = self.image_status_in_bucket(continuation_instruction_image)
            except Exception:
                raise Exception(f"Error assigning image status for instruction continuation sheets {continuation_instruction_image}")
        ...
        for index in range(self.continuation_sheet_preferences_count):
            ...
            try:
                logger.debug(f'Checking continuation preference image status for: {continuation_preference_image}')
    ...
    def add_temp_images_to_bucket(self) -> bool:
        ...
        for image in self.images_to_check:
            try:
                ...
                logger.debug(f"Empty file '{image}' added to the '{self.bucket}' bucket.")
            except Exception as e:
                raise Exception(f"Error: Failed to add empty file '{image}' to the '{self.bucket}' bucket. {e}")
...
def get_healthcheck_response(event):
    logger.info(
        "OK", extra=get_event_details_for_logs(event, status=200)
    )
    return {
        "isBase64Encoded": False,
        "statusCode": 200,
