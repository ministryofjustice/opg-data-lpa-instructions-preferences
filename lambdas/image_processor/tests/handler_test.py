...
def test_init_function(image_processor):
    ...
    assert (
        image_processor.sirius_bucket
        == f'opg-backoffice-datastore-{os.getenv("TARGET_ENVIRONMENT")}'
    )
...
def test_download_scanned_images(image_processor, monkeypatch):
    # Define test data
    s3_urls_dict = {
        "lpaScans": {"locations": ["s3://my_bucket/my_scan.pdf"]},
...
def test_extract_instructions_and_preferences(image_processor, monkeypatch):
    ...
    monkeypatch.setattr(
        image_processor, "get_timestamp_as_str", get_timestamp_as_str_mock
    )
...
def test_get_ocr_matches(monkeypatch, form_operator, image_processor):
    ...
    monkeypatch.setattr(
        image_processor, "double_image_size", MagicMock(return_value=["img1", "img2"])
    )
    monkeypatch.setattr(
    ...
    result = image_processor.get_ocr_matches(
    ...
    image_processor.match_first_form_image_text_to_form_meta.assert_called_once_with(
        "/path/to/form/meta", ["text1", "text2"], form_operator
    )
    image_processor.mixed_mode_page_identifier.assert_called_once_with(
...
def test_get_matching_continuation_items(image_processor, form_operator, monkeypatch):
    ...
    monkeypatch.setattr(
        image_processor, "get_preprocessed_images", MagicMock(return_value=[])
    )
    monkeypatch.setattr(
    ...
    # Call the function
    result = image_processor.get_matching_continuation_items(
    ...
    # Assertions
    assert (
        image_processor.get_preprocessed_images.call_count == 2
...
def test_find_instruction_and_preference_paths(image_processor, monkeypatch):
    ...
    result = image_processor.find_instruction_and_preference_paths(
    ...
    monkeypatch.setattr(
        image_processor, "detect_marked_checkbox", detect_marked_checkbox_mock
    )
    result = image_processor.find_instruction_and_preference_paths(
        path_selection, paths
    )
...
def test_get_continuation_sheet_paths(image_processor, monkeypatch):
    ...
    monkeypatch.setattr(
        image_processor, "detect_marked_checkbox", mock_detect_marked_checkbox
    )
    ...
    result = image_processor.get_continuation_sheet_paths(
...
def test_get_selected_paths_for_upload(image_processor):
    ...
    with patch.object(
        ...
        with patch.object(
            image_processor, "get_continuation_sheet_type", return_value="BOTH"
        ):
            with patch.object(
                image_processor,
                ...
                result = image_processor.get_selected_paths_for_upload(
...
def test_merge_continuation_images_into_path_selection(image_processor):
    ...
    output = image_processor.merge_continuation_images_into_path_selection(
...
def test_all_mandatory_fragments_and_one_of_fragments_exist(image_processor):
    ...
    result = image_processor.string_fragments_in_string(
...
def test_detect_marked_checkbox(image_processor):
    ...
    assert image_processor.detect_marked_checkbox(unmarked_checkbox_path) is True

    # Test an unmarked checkbox
    marked_checkbox_path = "/function/tests/checkbox_images/checkbox_blank.jpg"
    assert image_processor.detect_marked_checkbox(marked_checkbox_path) is False
    ...
    assert image_processor.detect_marked_checkbox(unmarked_checkbox_path) is False
...
def test_create_scan_to_template_distances(
    monkeypatch, image_processor, mock_form_images_as_strings, mock_form_metastore
):
    ...
    distances = image_processor.create_scan_to_template_distances(
