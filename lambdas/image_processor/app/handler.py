...
class ImageProcessor:
    def __init__(self, event):
        ...
        self.key_prefix = self.target_environment if self.target_environment != 'integration' else 'development'
    ...
    def cleanup(self, downloaded_image_locations):
        ...
        fail_path = f"{self.extraction_folder_path}/fail/{self.folder_name}"
    ...
    def download_scanned_images(self, s3_urls_dict):
        ...
        if lpa_location:
            ...
            scan_location = f'{self.extraction_folder_path}/{path_parts["file_path"]}'
    ...
    def extract_instructions_and_preferences(self, image_locations):
        ...
        for key, image_location in image_locations.items():
            _ = form_operator.run_full_pipeline(
                ...
                pass_dir=f"{self.extraction_folder_path}/pass/{self.folder_name}",
                fail_dir=f"{self.extraction_folder_path}/fail/{self.folder_name}",
    ...
    def get_secret(self):
        ...
        Raises:
            ClientError
        """
        secret_name = f"{self.target_environment}/jwt-key"
        try:
