...
class PathSelectionService:
    ...
    def get_selected_paths_for_upload(self, paths, continuation_keys_to_use) -> dict:
        ...
        if not self.check_continuation_sheets_match_expected(continuation_sheets, continuation_sheet_type):
            logger.warning(
                f"Images extracted from Continuation Sheets do not match what is expected based on the checkbox. Continuation keys={continuation_keys_to_use}, Continuation sheet type={continuation_sheet_type}"
            )
