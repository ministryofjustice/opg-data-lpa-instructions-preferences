...
class ExtractionService:
    ...
    def mask_images(self, metastore, images):
        updated_images = []
        for image in images:
    ...
    def get_ocr_matches(
        ...
        if len(metastore) == 1:
            ...
            masked_images = self.mask_images(metastore, processed_images)
            final_images = self.smart_threshold_images(masked_images)

        for key, image in enumerate(final_images):
            cv2.imwrite(f"/tmp/output/output_image_{key}.jpg", image)

        logger.debug("Applying OCR to extract text from images...")
        form_images_text = form_operator.form_images_to_text(final_images)
        ...
        #     metastore, form_images_text, form_operator
        # )
        # logger.debug(
