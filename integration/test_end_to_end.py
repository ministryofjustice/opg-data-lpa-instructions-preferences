...
def call_api_gateway(url):
    session = boto3.Session()
    credentials = session.get_credentials()
    auth = get_request_auth(credentials)
    response = requests.request(
        method='GET', url=url, auth=auth
    )
...
def make_calls_and_assertions(response_type, setup_rest_url_part):
    for template, template_data in config.templates.items():
        ...
        assert len(response_object['signed_urls']) == len(template_data[response_type]['signed_urls'])
...
def test_collection_in_progress(setup_rest_url_part):
    make_calls_and_assertions('expected_collection_in_progress_response', setup_rest_url_part)
